...
KISSY.add(function (S, Node, Event, Audio) {
    'use script';
      /**
   * 使用维基百科（搜狗）历史上的今天按照以下格式填写，注意：不要把某某死了，灾难性事件放上去，不吉利。
   */
    var BIRTH = [
       '1756年:奥地利作曲家莫扎特诞辰。',
    ...
    S.augment(ItClass, {
        ...
        init: function (cfg) {
           ...
           this.initEvent();
           if (!!S.UA.webkit) {
             Node.one('#tips').html('祝你生日快乐！天天开心！越来越女神！');
           }
           if (!!S.UA.ipad) {
             Node.one('#tips').html('舍特，PAD上有BUG，请到PC上看，说好的MOBILE FIRST呢！哭！');
           }
           Node.one('#tips').show();
        },
        initEvent: function () {
        ...
        initShare: function() {
            ...
            var title = encodeURIComponent('浪漫铺子祝您天天开心!');
            var source = encodeURIComponent('致 亲爱的你！'); // 网站名称
            var windowName = 'share'; //
            var site = 'http://www.langmanpuzi.com';
        ...
        step1: function(speed) {
            var content = S.get('#ItClass').innerHTML;
        ...
        initAudio: function() {
           var self = this;
           var Melody = {
               back: 'http://tsmusic24.tc.qq.com/4830437.mp3'
               //音乐地址
           };
